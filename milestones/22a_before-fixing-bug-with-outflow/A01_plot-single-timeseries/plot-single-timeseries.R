...
processed_data_path <- sprintf("./data/single_%d.csv", selected_sim)

processed_data <- if (file.exists(processed_data_path)) {
    read.csv(processed_data_path)
} else {
    arrow_ds <- open_dataset("../data/timeseries.arrow", format = "arrow")
    processed_data <- arrow_ds %>%
        filter(sim_number == selected_sim, integer %in% 1:10) %>%
        collect()
    dir.create(dirname(processed_data_path), recursive = TRUE, showWarnings = FALSE)
    write.csv(processed_data, processed_data_path, row.names = FALSE)
    processed_data
}
...
plot_title <- TeX(sprintf("Simulation %d: $log_{10}(I)=%.2f$, $log_{10}(k_d)=%.2f$", 
                           selected_sim, log10(sim_params$inflow_mols), log10(sim_params$diffusion_rate)))

p <- p + ggtitle(plot_title) + theme(plot.title = element_text(hjust = 0.5))

fn <- sprintf("figs/single-timeseries_%d.pdf", selected_sim)
ggsave(fn, plot = p, width = 8, height = 8, create.dir = TRUE)
