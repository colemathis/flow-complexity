...
TITLE        <- "Assembly whole system."
ID           <- "assembly-whole-system"
selected_sim <- 33
USE_CACHE   <- TRUE
...
merge_chemostats <- function(data) {
  data %>%
...
plot_timeseries <- function(data, grid_size, sim_params, sim_id) {
    ...
    ggplot(data, aes(x = time, y = total_assembly)) +
        geom_line(alpha = 0.7) +
        scale_y_log10(labels = scales::label_scientific()) +
        theme_bw() +
        theme(
...
processed_data <- merge_chemostats(processed_data)
