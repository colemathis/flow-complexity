...
ts_allsims = copy(pop)
filter!(row -> row.chemostat_id == 1, ts_allsims)
...
ts_multi = copy(pop)
filter!(row -> row.sim_number == 10, ts_multi)

p = plot(layout = grid(5, 5), size = (900, 900), legend=nothing, link=:both)

nspecies = 10
for i in unique(ts_multi.chemostat_id)
