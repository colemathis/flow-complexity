...
ID         				<<- "35b-detection-threshold-int-outflow"
USE_CACHE  				<<- TRUE
PRINT_FIGS 				<<- TRUE
SAVE_FIGS 			 	<<- TRUE
...
plot_figure <- function(detection_thresholds) {
  ...
  p <- p + scale_color_manual(values = cols, name = "Sensitivity log[X]")
  ...
  p <- p + theme(
	...
	# 	begin = 0.0,        # start slightly lighter than pure black
	# 	end = 0.3,          # stop before pure white
